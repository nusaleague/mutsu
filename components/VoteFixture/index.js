...
export class VoteFixture extends React.Component {
  ...
  handleSubmit = () => {
    ;(async () => {
      ...
      const submitData = {
        ...
        responses: Object.entries(this.state.choices).map(
          ([matchId, mascotId]) => ({
  ...
  render() {
    ...
    const divisionMatches = groupBy(
      data.vote_match.map(match => ({
        ...
        mascots: data.vote_match_mascot
          ...
          .map(matchMascot => {
            const mascot = data.mascot.find(
              mascot => mascot.id === matchMascot.mascot_id
            )
    ...
    const isAllChosen = !data.vote_match.find(
      match => !chosenIds.includes(String(match.id))
    )
      ...
      <>
        <Container>
          <Card className="card-fixture card-fixture-intro">
            <CardImg
              src={`${process.env.FILE_URL}/nusaimoe2019-banner.jpg`}
              alt="Nusaimoe banner"
            />
            <CardBody>
              <p>Selamat datang di Nusaimoe!</p>
              <p>
                Tentukan maskot favorit kalian dengan berpartisipasi dalam
                pertandingan mingguan dari setiap divisi Nusaimoe.
              </p>
            </CardBody>
          </Card>

          {Object.entries(divisionMatches).map(([division, matches]) => (
            <VoteDivision
              key={division}
              division={division}
              matches={matches}
          ...
          <Card className="card-fixture">
            <CardBody>
              {isAllChosen ? (
                <>
                  <p>
                    Terima kasih sudah memilih para maskot pada pertandingan
                    minggu ini!
                  </p>
                  <p>
                    Silakan tinggalkan komentar, kritik, atau saran kamu terkait
                    pertandingan kali ini di bawah ini.
                  </p>
                  <FormGroup>
                    <Input
                      type="textarea"
                ...
                <>
                  <p>
                    Silakan melengkapi pilihan kamu untuk
                    pertandingan-pertandingan di atas.
                  </p>
                  <Button disabled block type="button">
                    Kirim vote
                  </Button>
                </>
              )}
...
export default connect(state => {
  ...
  withPersist(props => `fixture-choices-${props.data.vote_fixture.id}`, {
    choices: {},
    comment: ''
  })(VoteFixture)
)
