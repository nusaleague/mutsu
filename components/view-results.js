...
export default class ViewResults extends React.Component {
  ...
  componentDidMount() {
    acall(async () => {
      ...
      const stages = await rpc('getAllFinishedStages', [
        seasons[0].comp,
        seasons[0].year
      ])
          ...
          this.state.result
            ? (() => {
                ...
                for (const matchMascot of result.matchMascot) {
                  const mascotRow = result.mascotRows.find(
                    row => row.id === matchMascot.mascot_id
                  )
                ...
                for (const match of Object.values(matches)) {
                  divisionMatches[
                    result.mascotSeasonRows.find(
                      row => row.mascot_id === match[0].mascot_id
                    ).division
                  ].push(match)
                }

                return Object.entries(divisionMatches).map(
                  ([division, matches]) => {
                    return (
                      <div key={division}>
                        <h1>
                          Divisi {division.replace(/^./, x => x.toUpperCase())}
                        </h1>
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="mascot-image left">
                                  <img
                                    src={`${process.env.FILE_URL}/banner/${matchLeft.mascotSlug}.png`}
                                  />
                                </span>
                                <span className={`score ${division}`}>
                                  {matchLeft.full_score} -{' '}
                                  {matchRight.full_score}
                                </span>
                                <span className="mascot-image right">
                                  <img
                                    src={`${process.env.FILE_URL}/banner/${matchRight.mascotSlug}.png`}
                                  />
                                </span>
